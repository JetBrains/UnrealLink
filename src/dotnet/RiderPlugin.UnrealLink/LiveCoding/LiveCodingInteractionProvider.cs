...
namespace RiderPlugin.UnrealLink.LiveCoding
{
  ...
  public class LiveCodingInteractionProvider : ILiveCodingProvider
  {
    ...
    public IRdTask<bool> HasStarted() =>
      _riderToEditor.EditorModel.LC_HasStarted.Start(_lifetimeDef.Lifetime, Unit.Instance);

    public IRdTask<bool> IsCompiling() =>
      _riderToEditor.EditorModel.LC_IsCompiling.Start(_lifetimeDef.Lifetime, Unit.Instance);

    public IRdTask<bool> CanEnableForSession() =>
      _riderToEditor.EditorModel.LC_CanEnableForSession.Start(_lifetimeDef.Lifetime, Unit.Instance);

    public IRdTask<bool> IsEnabledForSession() =>
      _riderToEditor.EditorModel.LC_IsEnabledForSession.Start(_lifetimeDef.Lifetime, Unit.Instance);

    public IRdTask<bool> IsEnabledByDefault() =>
      _riderToEditor.EditorModel.LC_IsEnabledByDefault.Start(_lifetimeDef.Lifetime, Unit.Instance);
